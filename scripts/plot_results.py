...
def make_subsample_figure(
    ...
    df_adapters_ss = df_adapters_ss.replace({**model_names, **task_names}).query(
        "model==['CLMBR_MIMIC', 'CLMBR_DAPT'] and task==@tasks"
    )
    ...
    if plot_deltas:
        ...
        base = data.query("model=='CLMBR_MIMIC'")
...
def make_subsample_figure_new(
    ...
    df_adapters_ss = pd.read_csv(path_to_csv_ss)
    df_adapters = df_adapters.replace({**model_names, **task_names}).query(
        "model=='CLMBR_MIMIC' and task==@tasks"
    )
...
if __name__ == "__main__":
    ...
    if benchmark_tables:
        ...
        # supplementary table [replace GBM with CLMBR_MIMIC]
        path_to_csv = os.path.join(path_results, "adapter_models/results.csv")
        models = ["CLMBR_MIMIC", "CLMBR", "CLMBR_DAPT"]
        ...
        for metric in ["AUROC", "AUPRC", "AUPRC_C", "ECE"]:
            ...
            df.to_csv(os.path.join(path_output_dir, f"supp_clmbr_mimic_{metric}.csv"))

    if benchmark_figures:
        # run fun for each metric
        # add supplementary (CLMBR_MIMIC instead of GBM)
        ...
        figure_sets = {
            ...
            "supp_clmbr_mimic": ["CLMBR_MIMIC", "CLMBR", "CLMBR_DAPT"],
        }
