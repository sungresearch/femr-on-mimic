...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="/hpf/projects/lsung/phi/data/mimic_omop_parquet/",
        help=(
    ...
    parser.add_argument(
        "--gcp_project_name",
        type=str,
        default="mimic-iv-ches",
        help=(
            "The name of *YOUR* GCP project (e.g. 'mimic-iv-ches')."
    ...
    parser.add_argument(
        "--gcp_dataset_id",
        type=str,
        default="mimiciv_full_current_cdm_54",
        help=(
            "The Dataset ID of the GCP dataset to download"
            " (e.g. 'mimiciv_full_current_cdm_54')."
    ...
    scratch_bucket_name = (
        args.gcp_project_name.replace("-", "_") + args.scratch_bucket_postfix
    )
    ...
    try:
        ...
        bucket = storage_client.create_bucket(scratch_bucket_name)
    ...
    extract_config = bigquery.job.ExtractJobConfig(
        ...
        destination_format=bigquery.job.DestinationFormat.PARQUET,
        # field_delimiter=",",
    )
    ...
    def download(table_id: str, f):
        """Download the results of the BigQuery extract job to our local filesystem
        Note that a single table will be extracted into possibly dozens of smaller files
    ...
    for table in tables:
        ...
        bucket_target_path: str = (
            f"gs://{scratch_bucket_name}/{random_dir}/{table.table_id}/*.parquet"
        )
