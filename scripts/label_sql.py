...
def register_omop_tables(c: Context, path_omop: str, file_format: str = "parquet"):
    ...
    for table in list_dir(path_omop):
        dfs[table] = dd.read_parquet(os.path.join(path_omop, table))
        c.create_table(table, dfs[table])
    return c
...
def create_ip_cohort(c):
    ...
    SELECT
        ...
        YEAR(visit_start_datetime) - p.year_of_birth as age_at_admission
    ...
    # adults only
    visits = visits.query("age_at_admission>=18")
    ...
    WHERE
        ...
        -- remove deaths that occurred prior to admission (used as index datetime, see below)
        (
